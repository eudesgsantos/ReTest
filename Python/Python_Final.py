...
while True:
    ...
    if res not in strokes:
        ...
        print(strokes)
    if len(strokes)>=3:
        timestamp = 1559839862
        tempo = str(datetime.fromtimestamp(timestamp))
        crac = strokes[0]
        ...
        pedido = indv+' pediu '+problema+' no(a) '+lugar+' quem vai resolver?'
        ...
        print('sai do bot')
        time.sleep(1)
        bot.polling(none_stop=False,interval=0,timeout=123)
        while True:
            pass
